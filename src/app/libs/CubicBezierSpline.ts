/**
 * Copyright 2012-2019 Jim Armstrong (www.algorithmist.net)
 *
...
/**
 * Typescript Math Toolkit, Cubic Bezier Spline. A cubic bezier spline that interpolates a series of points.  Each curve segment in between
 ...
 *
 */
import { TSMT$BezierSplineControl } from './BezierSplineControl';
import { TSMT$CubicBezier         } from './CubicBezier';
import {
  IControlPoints,
  IPoint
} from './IPlanarCurve';

export class TSMT$CubicBezierSpline
{
  protected _tangents: TSMT$BezierSplineControl = new TSMT$BezierSplineControl(); // spline control points (tangent manager)
  protected _bezier: Array<TSMT$CubicBezier>;                                     // list of each cubic Bezier in the spline
  ...
  constructor()
  {
    ...
    this._bezier      = new Array<TSMT$CubicBezier>();
    this._t           = 0;
    this._s           = 0;
    this._index       = 0;
    this._arcLength   = 0;
  ...
  */
  ...
  {
    const pointArr: Array<IPoint> = new Array<IPoint>();
    let i: number                 = 0;
  ...
  *
  * @param i Segment index in [0, #points-1]
  */
  public getCubicSegment(i: number): TSMT$CubicBezier | null
  {
    if (this._invalidated )
    {
      this.__tangents();
  ...
  *
  * @param x x-coordinates of new point set
  *
  * @param y y-coordinates of new point set
  */
  ...
  *
  * @param t Natural parameter in [0,1].  The input value is clipped if it is outside this interval
  */
  public getX(_t: number): number
  {
    ...
    const bezier: TSMT$CubicBezier = this._bezier[this._index];
