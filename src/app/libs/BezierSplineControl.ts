...
/**
 * Typescript Math Toolkit. BezierSplineControl computes tangents and thus geometric constraints for a collection of cubic Bezier segments 
 ...
 *
 */
export class TSMT$BezierSplineControl
{
  protected static ZERO_TOL: number = 0.000001; // zero-tolerance, suitable for this algorithm
  ...
  *
  * @param t Desired tension in [0,1], which actually varies the internal tension parameter between a
  ...
  *
  * @param i Zero-based segment index (there are #points - 2 segments)
  *
  */
  ...
  *
  * @param x x-coordinates of spline control or interpolation points
  * @param y y-coordinates of spline control or interpolation points
  */
  ...
  protected __segmentsCoef(i: number, t: number, x: Array<number>, y: Array<number>): void
  {
    ...
    const coef: IControlPoints = this._segments[i];
    coef.x0                    = x[i];
    coef.y0                    = y[i];
    coef.cx                    = this._bXNL;
    coef.cy                    = this._bYNL;
  ...
  protected __left(t: number, x: Array<number>, y: Array<number>): void
  {
    ...
    const coef: IControlPoints = this._segments[0];
